  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      const scaledMaxVelocity =
        config.maxVelocity * (0.1 + 0.9 * transitionFactor);
...
const DEFAULT_SPHERES_CONFIG: SpheresConfig = {
  ...
  ambientIntensity: 0.8, // Increase from 0.3 to 0.8
  lightIntensity: 300, // Increase from 150 to 300
  materialParams: {
    metalness: 0.3, // Increase from 0.1 to 0.3 for more reflection
    roughness: 0.2, // Decrease from 0.3 to 0.2 for more shine
    clearcoat: 1.0, // Increase from 0.5 to 1.0 for maximum clearcoat
    clearcoatRoughness: 0.05, // Decrease from 0.1 to 0.05 for smoother clearcoat
    transparent: false, // Change from true to false
    opacity: 1.0,
  },
  minSize: 0.5,
  maxSize: 1,
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<SpheresConfig> = {}) {
    ...
    const material = new Y({
      ...
      envMap,
      envMapIntensity: 1.25, // tweak 0.8–1.6 to taste
    });
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0 && this.config.followCursor === false) {
        ...
        const transitionScale = this.physics.isTransitioning
          ? baseScale * (0.1 + 0.9 * appearanceFactor)
          : baseScale;
...
function createBallpit(
  ...
  threeInstance.renderer.toneMappingExposure = 1.6;
